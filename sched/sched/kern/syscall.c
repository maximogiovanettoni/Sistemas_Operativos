...
static int
sys_env_set_priority(envid_t envid, int new_priority)
{
	struct Env *env;
	if (envid2env(envid, &env, 1) < 0) {
		return -1;
	}
	if (new_priority > env->priority && new_priority < MIN_PRIORITY) {
		return -1;
	}
...
sys_exofork(void)
{
	...
	newenv->priority = DEFAULT_PRIORITY;
	newenv->priority_counter = 0;
