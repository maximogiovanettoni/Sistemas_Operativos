...
{
	if (arg[0] != '$') {
		return arg;  // No es una variable de entorno.
	}
	if (strcmp(arg, "$?") == 0) {
	...
	if (path == NULL) {
		free(arg);          // Liberar el argumento original
		return strdup("");  // Retorna una cadena vacía (sin espacios)
	}

	size_t sizeValue = strlen(path);
	char *expansion = (char *) malloc(
	        (sizeValue + 1) *
	        sizeof(char));  // Reservar espacio para la expansión
	if (expansion == NULL) {
		perror("Failed to allocate memory");
		exit(EXIT_FAILURE);
	}

	strcpy(expansion, path);  // Copiar el valor expandido
	free(arg);                // Liberar el argumento original
	return expansion;         // Retornar la expansión
}
...
parse_exec(char *buf_cmd)
{
	...
	while (buf_cmd[idx] != END_STRING) {
		...
		if (tok[0] != '\0') {
			c->argv[argc++] = tok;  // Agregar el token expandido
		} else {
			free(tok);  // Liberar si la expansión fue vacía
		}
	}
